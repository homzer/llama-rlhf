...
from src.str_utils import deduplicate
from src.tokenizer import LlamaTokenizer
from src.trainer import ParallelSolverTrainer
from src.utils import setup_model_parallel, Timer, json_dump, json_load
...
class EnhanceDataset(JsonDataset):
    ...
    def enhance(self, enhance_datalist):
        for data in enhance_datalist:
            ...
            self.enhance_datalist[i]['output'] = deduplicate(self.enhance_datalist[i]['output'])
...
def run(
    ...
    policy_model = LoraLlama(policy_args)
    optimizer = torch.optim.Adam(policy_model.parameters(), lr=lr)
    trainer = ParallelSolverTrainer(policy_model, tokenizer, optimizer, max_seq_len)
    evaluator = SolverEvaluator(policy_model, tokenizer, eval_batch_size, max_seq_len)
    trainer.load(policy_ckpt_dir)

    for epoch in range(epochs):

        # Policy Model Collecting
        policy_buffer_collector = ActorBufferCollector(policy_model, tokenizer, max_seq_len)
        ...
        for data in tqdm(eval_dataloader):
            ...
            policy_rollout_buffer.extend(
                policy_buffer_collector.forward(data['instruction'])
            )
        ...
        for policy_data, reward_data in zip(policy_rollout_buffer.get(1), reward_rollout_buffer.get(1)):
            if reward_data.scores.mean() == 0:  # incorrect
                error_rollout_buffer.extend(
                    ActorRolloutBuffer(
                        instructions=policy_data.instructions,
                        obs=policy_data.obs,
                        actions=policy_data.actions,
                        action_logits=policy_data.action_logits,
                        action_masks=policy_data.action_masks
                    )
                )

        # Policy Model Revising
        revise_buffer_collector = ActorBufferCollector(policy_model, tokenizer, max_seq_len)
        revise_rollout_buffer = ActorRolloutBuffer()
        print("Policy model revising ...")
