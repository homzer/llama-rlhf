...
def rearrange_buffers_and_group_scores(
    ...
    return policy_rollout_buffer, verifier_rollout_buffer, scores
...
def compute_grpo_rewards(
    ...
    normed_scores = (scores - scores.mean(axis=-1, keepdims=True)) / scores_std
    normed_scores = np.where(normed_scores == 0, scores, normed_scores)
    normed_scores = normed_scores.reshape(-1).tolist()
    # update verifier rollout buffer
    verifier_rollout_buffer = ParallelRolloutBuffer(**CriticRolloutBuffer(
        scores=normed_scores, action_masks=verifier_rollout_buffer["action_masks"]
    ))
