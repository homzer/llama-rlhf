...
def get_partition_size(model_parallel_rank, layer_id):
    if (
            model_parallel_rank in [0, 2, 4, 6] and layer_id < 30
    ) or (
            model_parallel_rank in [1, 3, 5, 7] and layer_id >= 30
    ):
        return 896
    else:
        return 768
...
def get_n_local_heads(model_parallel_rank, layer_id):
    if (
            model_parallel_rank in [0, 2, 4, 6] and layer_id < 30
    ) or (
            model_parallel_rank in [1, 3, 5, 7] and layer_id >= 30
...
class LlamaAttention30B(LlamaAttention):
    def __init__(self, args: LoraLlamaArgs, layer_id: int):
        ...
        self.n_local_heads = get_n_local_heads(self.args.model_parallel_rank, layer_id)

    def init_weights(self):
        self.wq = ColumnParallelLinearPartitioned(
            self.args.dim,
            get_partition_size(self.args.model_parallel_rank, self.layer_id),
            bias=False,
            gather_output=False,
