...
class ParallelRolloutBuffer(RolloutBuffer):
    ...
    def scatter_to_data_parallel_region(self):
        ...
        if self.size() % world_size != 0:
            self.rearrange(np.concatenate([np.arange(0, self.size()), np.arange(0, self.size() % world_size)]))
    ...
    @classmethod
    def load(cls, buffer_dir: str, start: int = 0, stop: int = None) -> "ParallelRolloutBuffer":
