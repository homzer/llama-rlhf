...
class Mistral3Tokenizer(Tokenizer):
    def __init__(self, model_dir: str):
        ...
        self.special_tokens = ["<s>", "</s>", "[SYSTEM_PROMPT]", "[/SYSTEM_PROMPT]", "[INST]", "[/INST]"]
        ...
        self.special_token_snapshots = [
            " ".join(str(i) for i in self.model.encode(t, add_special_tokens=False)) for t in self.special_tokens
        ]

    def process_special_tokens(self, s: str) -> List[int]:
        escaped_tokens = [re.escape(token) for token in self.special_tokens]
        pattern = '|'.join(escaped_tokens)
        sub_s = [s_ for s_ in re.split(f'({pattern})', s) if s_]
        encode = []
        for s_ in sub_s:
            if s_ in self.special_tokens:
                encode.append(self.special_token_ids[s_])
            else:
                encode.extend(self.model.encode(s_, add_special_tokens=False))
    ...
    def encode(self, s: str, bos: bool = False, eos: bool = False) -> List[int]:
        encode = self.process_special_tokens(s)
