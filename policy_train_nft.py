...
from src.ppo.trainer import ParallelNFTTrainerForCausalLM
from src.utils import json_load, print_current_func_args


def compute_nft_rewards(
    ...
    group_scores = scores.mean(axis=-1).repeat(repeats=num_samples_per_prompt)  # [b,]
    group_verifier_rollout_buffer = ParallelRolloutBuffer(**CriticRolloutBuffer(
        scores=group_scores.tolist(), action_masks=verifier_rollout_buffer["action_masks"]
    ))

    # filter group_scores in (0,1) and shuffle
    group_score_indices = np.where((group_scores > 0) & (group_scores < 1))[0]
    print(f"Filter {len(group_score_indices)} group scores from {len(group_scores)}")
    np.random.shuffle(group_score_indices)
    policy_rollout_buffer.rearrange(group_score_indices)
    verifier_rollout_buffer.rearrange(group_score_indices)
    group_verifier_rollout_buffer.rearrange(group_score_indices)

    policy_rollout_buffer.scatter_to_data_parallel_region()
    verifier_rollout_buffer.scatter_to_data_parallel_region()
    group_verifier_rollout_buffer.scatter_to_data_parallel_region()
    return policy_rollout_buffer, verifier_rollout_buffer, group_verifier_rollout_buffer
...
def train_nft(
    ...
    trainer = ParallelNFTTrainerForCausalLM(
        policy, optimizer, epsilon=epsilon, save_optim=save_optim, accumulation_steps=accumulation_steps)
...
def run(
    ...
    for epoch, datalist in IterationHandler(json_load(train_file), epochs, chunk_size, begin_epoch):
        ...
        policy_rollout_buffer, verifier_rollout_buffer, group_verifier_rollout_buffer = compute_nft_rewards(
            policy_rollout_buffer=policy_rollout_buffer,
            verifier_rollout_buffer=verifier_rollout_buffer,
            num_samples_per_prompt=num_samples_per_prompt
        )
        ...
        rollout_buffer = RolloutBuffer(
            ...
            group_rewards=group_verifier_rollout_buffer["scores"],
