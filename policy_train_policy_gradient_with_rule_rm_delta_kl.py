...
def evaluate_kl(
    ...
    collector = ActorForwardBufferCollector(
        actor=actor,
    ...
    for data in actor_rollout_buffer.get(max_forward_batch_size):
        rollout_buffer = collector.forward(data.instructions, data.responses)
        kls.append(masked_mean(
            np.abs(rollout_buffer["action_logprobs"] - data.action_logprobs),
            mask=data.action_masks
        ).item())
    print("Delta KL:", np.mean(kls))
